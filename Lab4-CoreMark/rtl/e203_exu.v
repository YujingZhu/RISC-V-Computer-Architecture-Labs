...
module e203_exu(
  ...
  // ===========================================================================
  // [Optimization] Load-Use Data Forwarding Logic
  // ===========================================================================
  `ifdef E203_CFG_SUPPORT_LSU_FWD
  // 1. Detect if we can forward: 
  //    Long-pipe WB is valid AND Dest Reg matches Source Reg AND Dest is not x0
  //    Note: We only forward Integer registers (not FPU) for LSU load forwarding.
  wire fwd_rs1_en = longp_wbck_o_valid 
                  & (longp_wbck_o_rdidx == i_rs1idx) 
                  & (|longp_wbck_o_rdidx)
                  & (~longp_wbck_o_rdfpu); // Ensure it is integer write-back
  ...
  // 2. Select Data: Forwarded data or RegFile data
  //    longp_wbck_o_wdat is [FLEN-1:0], for RV32 it is compatible with XLEN
  ...
  // 3. Mask Dependency (Spoofing):
  //    If we forwarded, tell Dispatch "No OITF dependency conflict" for this operand.
  //    oitfrd_match_disprs1 comes from OITF module (calculated based on idx).
  ...
  e203_exu_disp u_e203_exu_disp(
    ...
    `ifdef E203_CFG_SUPPORT_LSU_FWD
    .disp_i_rs1          (disp_fwd_rs1    ),
    .disp_i_rs2          (disp_fwd_rs2    ),
    `else
    .disp_i_rs1          (rf_rs1          ),
    .disp_i_rs2          (rf_rs2          ),
    `endif
    ...
    `ifdef E203_CFG_SUPPORT_LSU_FWD
    .oitfrd_match_disprs1(disp_no_stall_rs1),
    .oitfrd_match_disprs2(disp_no_stall_rs2),
    `else
